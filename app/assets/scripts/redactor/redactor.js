  ...
  Redactor.fn = $.Redactor.prototype = {
    ...
    clean: function()
    {
      return {
        onSet: function(html)
        {
          ...
          // save markers
          html = html.replace(/<span(.*?[^>]?)id="selection-marker-1"(.*?[^>]?)>â€‹<\/span>/gi, '[[[marker1]]]');
          html = html.replace(/<span(.*?[^>]?)id="selection-marker-2"(.*?[^>]?)>â€‹<\/span>/gi, '[[[marker2]]]');
          ...
          // restore markers
          html = html.replace('[[[marker1]]]', '<span id="selection-marker-1" class="redactor-selection-marker">â€‹</span>');
          html = html.replace('[[[marker2]]]', '<span id="selection-marker-2" class="redactor-selection-marker">â€‹</span>');
        ...
        {
          html = html.replace(/&lt;span id=&quot;selection-marker-([0-9])&quot; class=&quot;redactor-selection-marker&quot;&gt;â€‹&lt;\/span&gt;/g, '<span id="selection-marker-$1" class="redactor-selection-marker">â€‹</span>');
    ...
    code: function()
    {
      return {
        ...
        {
          html = $.trim(html);
          html = html.replace(/^(<span id="selection-marker-1" class="redactor-selection-marker">â€‹<\/span>)/, '');
    ...
    inline: function()
    {
      return {
        ...
        insertBreakpoint: function(inline, currentTag)
        {
          ...
          code = code.replace(/<span\sid="redactor-inline-breakpoint">â€‹<\/span>/i, '</' + currentTag + '>' + endTag);
          var $code = $(code);
    ...
    modal: function()
    {
      return {
        ...
        disableEvents: function()
        {
          ...
          this.$modalBox.off('clickÂ§.redactor-modal');
